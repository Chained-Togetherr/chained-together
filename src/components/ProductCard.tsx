  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-[hsl(38,42%,97%)]" style={{ aspectRatio: "1 / 1" }}>
        ...
        {/* Gradient overlay on hover (only when not sold out) */}
        {!isSoldOut && (
        ...
        {isLimited && (
          ...
          >
            {/* Dot indicator */}
            <span
              className="w-1 h-1 rounded-full bg-white/80 inline-block animate-pulse"
              style={{ flexShrink: 0 }}
            />
            LIMITED
          </div>
        ...
        {/* Quick-add button on desktop hover (hidden when sold out) */}
        {!isSoldOut && (
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {product.hasVariants && !isSoldOut && (
          <p
            className="text-[10px] text-muted-foreground/60 mb-3 tracking-wide uppercase"
            style={{ fontFamily: "DM Sans, sans-serif", letterSpacing: "0.09em" }}
          >
            Pilih Varian
          </p>
        )}

        {/* Desktop: price + add button side by side */}
        <div className="hidden sm:flex items-center justify-between mt-auto pt-2">
          <div>
            <span
              ...
              style={{
                background: isSoldOut
                  ? "none"
                  : "linear-gradient(135deg, hsl(345 55% 60%), hsl(345 50% 52%))",
                WebkitBackgroundClip: isSoldOut ? "unset" : "text",
                WebkitTextFillColor: isSoldOut ? "hsl(0 0% 60%)" : "transparent",
                color: isSoldOut ? "hsl(0 0% 60%)" : undefined,
                fontFamily: "DM Sans, sans-serif",
            ...
            {product.bellOption && !isSoldOut && (
              <span
                className="block text-[9px] text-muted-foreground/55 mt-0.5"
          ...
          {!isSoldOut ? (
            <motion.button
              ...
              style={{
                background: "linear-gradient(135deg, hsl(345 55% 68%), hsl(345 50% 58%))",
                boxShadow: "0 3px 12px -2px hsl(345 55% 68% / 0.45)",
              }}
        ...
        <div className="sm:hidden mt-auto pt-2">
          ...
          {!isSoldOut ? (
            <motion.button
              whileTap={{ scale: 0.97 }}
              onClick={() => onAddToCart(product)}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-full text-white text-sm font-medium transition-all duration-300"
              style={{
                background: "linear-gradient(135deg, hsl(345 55% 68%), hsl(345 50% 58%))",
                fontFamily: "DM Sans, sans-serif",
                boxShadow: "var(--shadow-button)",
            ...
            <div
              className="flex items-center justify-center w-full py-2.5 rounded-full border text-sm font-medium"
              style={{
                fontFamily: "DM Sans, sans-serif",
                color: "hsl(0, 70%, 55%)",
